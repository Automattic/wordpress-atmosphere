...
class Test_Publisher extends WP_UnitTestCase {
	...
	/**
	 * Visibility cleanup must delete a bsky record even when only the
	 * locally-reserved TID exists.
	 */
	public function test_delete_post_includes_bare_bsky_tid_for_visibility_cleanup() {
		$post = self::factory()->post->create_and_get(
			array( 'post_status' => 'trash' )
		);

		\update_post_meta( $post->ID, Post::META_TID, 'bare-bsky-tid' );
		\update_post_meta( $post->ID, Document::META_TID, 'doc-tid-456' );
		...
		$this->assertSame( 'com.atproto.repo.applyWrites#delete', $writes[0]['$type'] );
		$this->assertSame( 'app.bsky.feed.post', $writes[0]['collection'] );
		$this->assertSame( 'bare-bsky-tid', $writes[0]['rkey'] );
		$this->assertSame( 'site.standard.document', $writes[1]['collection'] );
		$this->assertSame( 'doc-tid-456', $writes[1]['rkey'] );
	}

	/**
	 * If a post becomes non-publishable while applyWrites is in flight,
	 * Publisher removes the records it just wrote.
	 */
	public function test_publish_post_reconciles_when_post_protected_mid_publish() {
		$post = self::factory()->post->create_and_get(
			array(
				'post_status'  => 'publish',
		...
		$this->assertIsArray( $result );
		$this->assertCount( 2, $captured_calls, 'Publish must be followed by cleanup.' );
