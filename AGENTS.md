...
**Connectors API** — Progressive-enhancement registration on the WordPress 7.0 Settings → Connectors screen. `Atmosphere\Connectors` (`includes/class-connectors.php`) registers an `atmosphere` connector on `wp_connectors_init` with `authentication.method => 'none'` and ships a script module (`src/connectors-card/`) that renders the connect/disconnect card, since core only auto-renders a UI for `api_key` connectors. The card drives the flow through `Atmosphere\Rest\Admin\Connection_Controller` (authorize/disconnect), reusing the existing OAuth `Client`. Guard the whole integration with `class_exists( 'WP_Connector_Registry' )`.

The screen has two possible URLs: core's top-level `options-connectors.php` (`Connectors::SCREEN`) and the Gutenberg plugin's Settings submenu (`options-general.php?page=options-connectors-wp-admin`). Both are matched via the shared `Connectors::SCREEN_SLUG` marker — `Connectors::is_connectors_screen()` gates the card enqueue, `Connectors::screen_url()` resolves the OAuth return destination. No URL is ever taken off the wire: the card sets a boolean `atmosphere_oauth_from_connectors` transient, and `Admin::handle_oauth_callback()` picks the destination server-side (`screen_url()` reads the registered admin `$submenu`), so nothing external can steer the `wp_safe_redirect()`.

The card's handle field offers typeahead via the `atmosphere_handle_typeahead_url` filter (default `public.api.bsky.app`; `''` disables it), shared with the plugin's own Settings connect field through `Atmosphere\handle_typeahead_url()` and `src/shared/handle-typeahead.js`.

**Embedding as a connection layer** — Two filters let a host plugin reuse just the connection: `atmosphere_show_settings_page` (via `Admin::is_settings_page_visible()`) hides the plugin's own settings page — it stays *registered* (`add_options_page`) because the OAuth callback lands on its URL, but drops from the menu. `atmosphere_connection_only_mode` (via `Atmosphere\is_connection_only_mode()`, default `false`) is the broader switch: it forces cross-posting, reaction sync, reply sync, and comment publishing **off**, skips scheduling the `atmosphere_sync_reactions` cron, and defaults the settings page hidden. Each behavior resolves through a dedicated helper (`is_auto_publish_enabled()`, `is_reaction_sync_enabled()`, `is_reply_sync_enabled()`, `is_comment_publishing_enabled()`, `Admin::is_settings_page_visible()`) that reads the stored option, forces it off in connection-only mode, then applies a per-feature filter **last** (`atmosphere_should_auto_publish`, etc.) — so a host can re-enable a single lane. All behavioral call sites route through these helpers; only the settings form fields read the raw option.
