...
**Connectors API** — Progressive-enhancement registration on the WordPress 7.0 Settings → Connectors screen. `Atmosphere\Connectors` (`includes/class-connectors.php`) registers an `atmosphere` connector on `wp_connectors_init` with `authentication.method => 'none'` and ships a script module (`src/connectors-card/`) that renders the connect/disconnect card via `method:none` + a custom render, since core only auto-renders a UI for `api_key` connectors. The card drives the flow through `Atmosphere\Rest\Admin\Connection_Controller` (authorize/disconnect), reusing the existing OAuth `Client`. The Connectors screen has two possible URLs for the same logical screen: stock core serves it as the top-level `options-connectors.php` (`Connectors::SCREEN`), while nav unification (the Gutenberg plugin, e.g. on WordPress.com/Atomic) re-registers it as a Settings submenu at `options-general.php?page=options-connectors-wp-admin`. Both are recognized through the shared `Connectors::SCREEN_SLUG` marker (`options-connectors`) rather than the exact filename — `Connectors::is_connectors_screen()` matches the card enqueue against either hook suffix (core's `options-connectors.php` and the remapped `settings_page_options-connectors-wp-admin`), and `Connectors::screen_url()` resolves the OAuth return destination. Even with two URLs, **no URL is ever passed around or validated off the wire**: the card supplies only the handle, the controller sets a boolean `atmosphere_oauth_from_connectors` transient on success, and the OAuth callback (`Admin::handle_oauth_callback()`) reads that flag and redirects to a destination resolved **server-side** — `Connectors::screen_url()`, which prefers the re-registered submenu URL when it is present in the registered admin menu (`$submenu`, fully built on `admin_menu` before the callback's `admin_init`) and otherwise falls back to `admin_url( Connectors::SCREEN )`, else the settings page. Because the flag is a boolean and the destination is computed from the trusted admin menu (never from request input), nothing off the wire can steer the post-callback `wp_safe_redirect()`. The settings-page connect (its own admin-post handler, not this REST route) clears the flag before starting so a stale card flow can't bounce it to the Connectors screen. Its handle field offers typeahead (an `app.bsky.actor.searchActorsTypeahead` XRPC endpoint), defaulting to Bluesky's official public appview (`public.api.bsky.app`) and centralized + filterable via `atmosphere_handle_typeahead_url` (point it at a network-wide index such as `typeahead.waow.tech`, or return `''` to disable typeahead and require manual entry). The same typeahead now also enhances the plugin's own Settings → ATmosphere connect field, sharing the `Atmosphere\handle_typeahead_url()` resolver (in `includes/functions.php`) and the `src/shared/handle-typeahead.js` component, so the `atmosphere_handle_typeahead_url` filter governs both surfaces. The enqueue and `wp_connectors_init` registration are guarded by `class_exists( 'WP_Connector_Registry' )`, so the card is inert where the Connectors screen isn't present and the plugin's own settings page stays the baseline. A plugin that embeds ATmosphere as a connection layer can hide that baseline: returning `false` from the `atmosphere_show_settings_page` filter (gated through `Admin::is_settings_page_visible()`) drops the page from the Settings menu (`remove_submenu_page`), suppresses the reauth `admin_notices` notice (its only CTA links to the hidden page), and turns the plugin-row `plugin_action_links` Settings shortcut into a plain "Settings hidden by another plugin" label. The page is still **registered** via `add_options_page` — it must be, because the OAuth callback lands on its own URL (`options-general.php?page=atmosphere`) and an unregistered page trips core's `user_can_access_admin_page()` gate, which `wp_die`s before `admin_init` runs `handle_oauth_callback()`; leaving it registered keeps that URL live so the callback can redirect on to the Connectors screen. A direct visit to the hidden page renders a short "managed by another plugin" notice instead of the settings form. Publishing, OAuth, and the Connectors card are untouched — that filter alone only hides the plugin's own settings UI. Hiding the UI while the plugin keeps auto cross-posting and importing reactions is a footgun for a host that wants ATmosphere purely as a connection layer, so there is a broader switch above the settings-page filter: `Atmosphere\is_connection_only_mode()` (in `includes/functions.php`), driven by the `atmosphere_connection_only_mode` filter (default `false`). When a host returns `true`, ATmosphere stops acting on its own — automatic cross-posting, like/repost import, reply import, and publishing local comments as Bluesky replies (the outgoing control added in #199) are all forced **off**, and `atmosphere_show_settings_page` now *defaults* to hidden (so no second filter is needed). It isn't just the writes: `Reaction_Sync::sync()` short-circuits at the top in connection-only mode, so the hourly `listNotifications`/`listRecords` polling never runs, and the `atmosphere_sync_reactions` cron is not scheduled in the first place (`Atmosphere::init()`). The short-circuit deliberately returns *before* the sync watermarks advance — unlike the per-setting toggles, which keep advancing so re-enabling doesn't backfill — so a host that later leaves connection-only mode picks up the interactions that arrived meanwhile rather than skipping them for good. The gating is layered, not all-or-nothing: each behavior resolves through a dedicated helper (`is_auto_publish_enabled()`, `is_reaction_sync_enabled()`, `is_reply_sync_enabled()`, `is_comment_publishing_enabled()`, `Admin::is_settings_page_visible()`) that reads the stored per-site option, forces it off in connection-only mode, then applies a per-feature filter **last** (`atmosphere_should_auto_publish`, `atmosphere_should_sync_reactions`, `atmosphere_should_sync_replies`, `atmosphere_should_publish_comments`, `atmosphere_show_settings_page`). Because the per-feature filter has the final say, a host can re-enable a single lane — e.g. keep cross-posting on while suppressing reactions. The override is on *effective* behavior (a hard short-circuit), not merely the option default, so the outcome does not depend on whether the site previously saved a value. The comment-publishing lane leans on #199's `is_comment_publishing_enabled()`, which is already re-checked at every layer (direct `Publisher` calls, the `atmosphere_publish_comment` cron, `on_before_delete` cleanup, and chunked `delete_post_by_tids` deletion), so forcing it off in the one helper propagates to all those call sites for free. All the behavioral call sites (`Atmosphere::on_status_change()` / `on_share_meta_changed()`, `Reaction_Sync`, comment publishing/cleanup, the pre-publish REST preview, the Site Health "Auto-Publish" row, the missing-post-types settings warning) route through those helpers rather than reading `get_option()` directly; only the settings *form fields* still read the raw stored option, since they edit the preference.
